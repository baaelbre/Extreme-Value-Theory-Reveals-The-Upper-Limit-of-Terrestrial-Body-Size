...
cat("  y*_SVL (log): [", ci_y1_boot_log[1], ", ", ci_y1_boot_log[2], "]\n", sep = "")
cat("  y*_TL  (log): [", ci_y2_boot_log[1], ", ", ci_y2_boot_log[2], "]\n", sep = "")

# Back-transform to original scale
ci_y1_boot_orig <- exp(ci_y1_boot_log)
ci_y2_boot_orig <- exp(ci_y2_boot_log)

cat("\n", 100 * ci_level, "% bootstrap CIs for endpoints (original scale):\n", sep = "")
cat("  SVL*: [", ci_y1_boot_orig[1], ", ", ci_y1_boot_orig[2], "]\n", sep = "")
cat("  TL* : [", ci_y2_boot_orig[1], ", ", ci_y2_boot_orig[2], "]\n", sep = "")
...
# ============================================================
# 11. Bootstrap endpoint distributions: SVL* and TL*
# ============================================================
...
hpdi_svl <- ci_y1_boot_orig
hpdi_tl  <- ci_y2_boot_orig

# Stokes values (already defined above: SVL_stokes, TL_stokes)
svl_stokes <- SVL_stokes
tl_stokes  <- TL_stokes

# Density estimates for smooth curves
svl_dens <- density(svl_boot_cm, n = 512)
tl_dens  <- density(tl_boot_cm,  n = 512)

svl_df      <- data.frame(SVL = svl_dens$x, density = svl_dens$y)
svl_samp_df <- data.frame(SVL = svl_boot_cm)

tl_df      <- data.frame(TL = tl_dens$x, density = tl_dens$y)
tl_samp_df <- data.frame(TL = tl_boot_cm)

# Reasonable x-limits based on bootstrap range
svl_xlim <- quantile(svl_boot_cm, c(0.01, 0.99), na.rm = TRUE)
tl_xlim  <- quantile(tl_boot_cm,  c(0.01, 0.99), na.rm = TRUE)
...
p_svl <- ggplot() +
  ...
  geom_vline(xintercept = map_svl,
             color = "purple", linetype = "dashed", size = 1.2) +
  geom_vline(xintercept = hpdi_svl,
...
p_tl <- ggplot() +
  ...
  geom_vline(xintercept = map_tl,
             color = "purple", linetype = "dashed", size = 1.2) +
  geom_vline(xintercept = hpdi_tl,
             color = "orange", linetype = "dotdash", size = 1.2) +
  geom_vline(xintercept = tl_stokes,
             color = "black", size = 1.0) +
  labs(x = "TL endpoint (cm)", y = "Density") +
  coord_cartesian(xlim = tl_xlim) +
  theme_science_polished
p_tl
# Optionally save:
