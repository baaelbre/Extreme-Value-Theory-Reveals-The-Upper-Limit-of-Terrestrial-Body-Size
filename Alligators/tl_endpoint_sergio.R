...
library(dplyr)
library(readxl)      # kept in case you load other files elsewhere
...
## ---------------------------
## 2) Data ingest (UPDATED)
## ---------------------------
# Set the path to the attached CSV (adjust if needed)
data_path <- "Data/CaptureData_Gator_allometry_paper_2024.csv"

dat_raw <- read.csv(data_path, stringsAsFactors = FALSE)

# Map to the names your pipeline expects; coerce obvious numeric columns
dat <- dat_raw %>%
  mutate(
    ...
    # Build a simple deformation flag from notes:
    # 0 = clean if empty/NA or contains "none"; 1 = deformed otherwise
    Deform = case_when(
...
# Optional sanity filters: drop obvious non-sense
dat <- dat %>%
  mutate(
...
make_thresholds <- function(y, q_from = 0.7, q_to = 0.999, n = 50){
...
## ---------------------------------------------------------
u_seq <- make_thresholds(y, q_from = 0.7, q_to = 0.99, n = 50)
...
p_mrl <- ggplot(data.frame(u = u_seq, mrl = mrl_vals), aes(u, mrl)) +
  ...
  theme_science_polished
p_mrl
ggsave(file.path(fig_dir, "tl_log_mrl_sergio.png"), p_mrl, dpi = 600, width = 7, height = 5, units = "in")
...
ggsave(file.path(fig_dir, "shape_stability_sergio.png"), p_shape, dpi = 600, width = 7, height = 5, units = "in")
...
p_adj
ggsave(file.path(fig_dir, "adj_scale_stability_sergio.png"), p_adj, dpi = 600, width = 7, height = 5, units = "in")
...
ggsave(file.path(fig_dir, "tl_log_qq_sergio.png"),  diags$pqq, dpi = 600, width = 7, height = 5, units = "in")
ggsave(file.path(fig_dir, "tl_log_pp_sergio.png"),  diags$ppp, dpi = 600, width = 7, height = 5, units = "in")
ggsave(file.path(fig_dir, "tl_log_pit_uniformity_sergio.png"), diags$pks, dpi = 600, width = 7, height = 5, units = "in")
...
mu_y <- log(430)
prior_mode <- "woodward_loose"  # loose|mod|tight (choose as you prefer)
sd_y <- switch(prior_mode,
               "woodward_loose" = 0.095,
               "woodward_mod"   = 0.048,
               "woodward_tight" = 0.024, 0.048)
logprior_y  <- function(y_star) dnorm(y_star, mean = mu_y, sd = 2*sd_y, log = TRUE)
...
# Prior on y* (log TL*) -> density on TL* via change of variables:
# f_TL(t) = f_Y(log t) * (1/t)
prior_y_dens   <- dnorm(y_star_grid, mean = mu_y, sd = 2*sd_y)  # matches your logprior_y
prior_tl_dens  <- prior_y_dens / tl_grid_cm

# Optional: renormalize prior on *this* plotting grid (safer since grid is truncated)
prior_Z        <- pracma::trapz(tl_grid_cm, prior_tl_dens)
if (is.finite(prior_Z) && prior_Z > 0) prior_tl_dens <- prior_tl_dens / prior_Z

df_tlplot <- data.frame(
  TL     = tl_grid_cm,
  post   = dens_tl,
  prior  = prior_tl_dens
)

# Rebuild the plot with both densities
p_tlstar <- ggplot(df_tlplot, aes(TL)) +
  # prior: light grey fill + thin outline
  geom_area(aes(y = prior), fill = "grey70", alpha = 0.18) +
  geom_line(aes(y = prior), linewidth = 0.8, color = "grey50", alpha = 0.85) +
  # posterior: dark blue line
  geom_line(aes(y = post),  linewidth = 1.2, color = "darkblue") +
  # reference & summaries
  ...
  annotate("text", x = stokes_tl_cm, y = max(df_tlplot$post, na.rm=TRUE)*0.11, vjust = -0.5,
           label = "Stokes (450 cm)", color = "firebrick", angle = 90, size = 4) +
  labs(
    x = "TL* (cm)", y = "Density",
    caption = "Grey: prior (Woodward on log TL*, mapped to TL). Blue: posterior."
  ) +
  xlim(350, 550) +
  theme_science_polished

# (Optional) mark prior mean on TL scale, if you like:
prior_mean_tl <- exp(mu_y)
p_tlstar <- p_tlstar +
  geom_vline(xintercept = prior_mean_tl, color = "grey40", linetype = "dotted") +
  annotate("text", x = prior_mean_tl, y = max(df_tlplot$post, na.rm=TRUE)*0.06, vjust = -0.8,
           label = "Prior mean", color = "grey40", angle = 90, size = 3.5)
...
ggsave(file.path(fig_dir, "tl_endpoint_posterior_sergio.png"),
...
## ---------------------------------------------------------
## 8) Mass extrapolation (sample a,b; sample y*; add residual quantile)
## ---------------------------------------------------------
...
p_mass <- ggplot(data.frame(W = mass_draws_kg), aes(W)) +
  ...
  annotate("text", x = stokes_w_kg, y = max(dens_m$y)*0.15, vjust = -0.6,
...
p_mass <- p_mass+
annotate("text",
         x = mass_map_kg, y = 0.002,
         label = "313 kg",
         size = 5, fontface = "bold",
         vjust = -0.5, angle = 90,
         color = "purple")
p_mass
ggsave(file.path(fig_dir, "mass_endpoint_sergio.png"),
...
p_reg <- ggplot(plot_df, aes(x = logTL, y = logW, color = Sex)) +
  ...
  theme_science_polished
p_reg
ggsave(file.path(fig_dir, "regression_panel_sergio.png"),
...
# Same helpers already computed above:
# ab <- coef(lm_w_tl)
# y_at_u0 <- as.numeric(predict(lm_w_tl, newdata = data.frame(logTL = u0)))
# x_ep <- ystar_map; x_ep_ub <- ystar_upper; y_ep <- log(mass_map_kg); y_ep_ub <- log(mass_up95_kg)
# x_stokes <- log(stokes_tl_cm); y_stokes <- log(stokes_w_kg)

p_reg_male <- ggplot(plot_df_male, aes(x = logTL, y = logW)) +
  geom_point(alpha = 0.8, size = 2.2, color = "#66b3ff") +             # blue male dots only
  geom_abline(intercept = ab[1], slope = ab[2], linewidth = 1) +       # same regression line
  geom_vline(xintercept = u0, linetype = "dashed", color = "gray30") + # threshold (x)
  geom_hline(yintercept = y_at_u0, linetype = "dashed", color = "gray30") + # threshold (y on reg line)
  geom_point(aes(x = x_ep, y = y_ep), color = "darkgreen", size = 3, inherit.aes = FALSE) +  # endpoint point
  geom_segment(aes(x = x_ep, y = y_ep, xend = x_ep_ub, yend = y_ep),   # one-sided x error bar
               color = "darkgreen", linewidth = 1.1, inherit.aes = FALSE) +
  geom_segment(aes(x = x_ep, y = y_ep, xend = x_ep, yend = y_ep_ub),   # one-sided y error bar
               color = "darkgreen", linewidth = 1.1, inherit.aes = FALSE) +
  geom_point(aes(x = x_stokes, y = y_stokes), inherit.aes = FALSE, color = "firebrick", size = 2.8) +
  geom_text(aes(x = x_stokes, y = y_stokes, label = "Stokes"),
            inherit.aes = FALSE, nudge_y = 0.015, color = "firebrick", size = 3.5) +
  labs(x = "log Total Length", y = "log Weight") +
  theme_science_polished +
  theme(legend.position = "none")   # no legend (only males shown)

p_reg_male

ggsave(file.path(fig_dir, "M_regression_panel_sergio.png"),
       p_reg_male, dpi = 600, width = 8, height = 5.2, units = "in")

## ---------------------------------------------------------
## 9) Exceedance probability of Stokes TL (reuses grids)
## ---------------------------------------------------------
