############################################################
# Hierarchical EVT for log TL exceedances (two datasets)
#   • Weibull-domain finite-endpoint GP (ξ ∈ (-1,0))
#   • Quadrature over (y*, ξ) and hyperparameters (μ_y, τ_y, μ_ξ, τ_ξ)
#   • Dataset posteriors, population predictive, and summary plots
# Author: Bastiaan Aelbrecht (refactor by ChatGPT)
# Date  : 2025-10-05
############################################################

## ---------------------------
## 0) Libraries & global setup
## ---------------------------
suppressPackageStartupMessages({
  library(ggplot2)
  library(dplyr)
  library(readxl)
  library(pracma)
  library(grid)
})
options(stringsAsFactors = FALSE)
set.seed(42)

## Figure dir and plot theme
FIG_DIR <- "Figures"
if (!dir.exists(FIG_DIR)) dir.create(FIG_DIR, recursive = TRUE)

theme_science_polished <- theme_minimal(base_family = "Arial", base_size = 12) +
  theme(
    axis.title       = element_text(size = 14, face = "bold"),
    axis.text        = element_text(size = 12),
    legend.title     = element_text(size = 10, face = "bold"),
    legend.text      = element_text(size = 10),
    panel.grid.major = element_line(color = "gray80", linewidth = 0.3),
    panel.grid.minor = element_blank(),
    panel.border     = element_rect(color = "black", fill = NA, linewidth = 0.6),
    axis.ticks.length= unit(0.20, "cm"),
    axis.ticks       = element_line(color = "black", linewidth = 0.4),
    plot.margin      = margin(5, 5, 5, 5),
    legend.position  = "right"
  )

## ---------------------------
## 1) CONFIG: thresholds, priors, grids
## ---------------------------
# Threshold quantiles for each dataset (Weibull POT)
q_s <- 0.95
q_a <- 0.95

# Reference & CI level
stokes_tl_cm <- 450
ci_level     <- 0.95

# Hyperprior centers / scales (weakly informative)
mu_y0   <- log(430)
sd_y0   <- 0.30
mu_xi0  <- -0.20
sd_xi0  <- 0.20
hc_scale_y  <- 0.5
hc_scale_xi <- 0.5

# Core grid sizes
Ny_per_group <- 320   # y* points per dataset (log TL)
Nxi          <- 200   # ξ points in (-1,0)
N_mu_y       <- 201
N_tau_y      <- 41
N_mu_xi      <- 61
N_tau_xi     <- 41

# Hyper grid bounds
mu_y_min  <- mu_y0 - 4*sd_y0
mu_y_max  <- mu_y0 + 4*sd_y0
tau_y_min <- 0.01
tau_y_max <- 3
mu_xi_min <- -0.90
mu_xi_max <- -0.01
tau_xi_min<- 0.01
tau_xi_max<- 3

eps <- 1e-300

## ---------------------------
## 2) Data loading utilities
## ---------------------------
safe_num <- function(x) suppressWarnings(as.numeric(x))

load_sergio <- function(path_csv) {
  raw <- read.csv(path_csv)
  raw %>%
    mutate(
      SVL      = safe_num(SV.length),
      TL       = safe_num(Total.Length),
      deform_s = case_when(
        is.na(Deformities_Notes) ~ 0L,
        nchar(trimws(Deformities_Notes)) == 0 ~ 0L,
        grepl("^none$", trimws(tolower(Deformities_Notes))) ~ 0L,
        TRUE ~ 1L
      )
    ) -> d
  
  # OLS to impute TL from SVL if needed (only non-deformed used)
  reg <- d %>% filter(deform_s==0L, is.finite(TL), is.finite(SVL)) %>%
    transmute(logTL = log(TL), logSVL = log(SVL))
  stopifnot(nrow(reg) >= 10)
  fit <- lm(logTL ~ logSVL, data = reg)
  
  d %>%
    mutate(
      logSVL    = ifelse(is.finite(SVL), log(SVL), NA_real_),
      logTL_obs = ifelse(is.finite(TL),  log(TL),  NA_real_),
      logTL_pred= ifelse(is.finite(logSVL),
                         as.numeric(predict(fit, newdata = data.frame(logSVL=logSVL))),
                         NA_real_),
      logTL     = ifelse(is.finite(logTL_obs), logTL_obs, logTL_pred)
    )
}

load_allan <- function(path_xlsx) {
  d <- read_excel(path_xlsx) %>%
    mutate(SVL = as.numeric(SVL),
           TL  = as.numeric(TL),
           Deform = as.integer(Deform))
  
  reg <- d %>% filter(Deform==0L, is.finite(TL), is.finite(SVL)) %>%
    transmute(logTL = log(TL), logSVL = log(SVL))
  stopifnot(nrow(reg) >= 10)
  fit <- lm(logTL ~ logSVL, data = reg)
  
  d %>%
    mutate(
      logSVL    = ifelse(is.finite(SVL), log(SVL), NA_real_),
      logTL_obs = ifelse(is.finite(TL),  log(TL),  NA_real_),
      logTL_pred= ifelse(is.finite(logSVL),
                         as.numeric(predict(fit, newdata = data.frame(logSVL=logSVL))),
                         NA_real_),
      logTL     = ifelse(is.finite(logTL_obs), logTL_obs, logTL_pred)
    )
}

## Load datasets
sergio_path <- "Data/CaptureData_Gator_allometry_paper_2024.csv"
allan_path  <- "Data/experimental_alligator_harvest_woodward.xlsx"

dat_s <- load_sergio(sergio_path)
dat_a <- load_allan(allan_path)

y_s <- dat_s$logTL[is.finite(dat_s$logTL)]
y_a <- dat_a$logTL[is.finite(dat_a$logTL)]
stopifnot(length(y_s) >= 50, length(y_a) >= 50)

## ---------------------------
## 3) Thresholds & exceedances
## ---------------------------
u_s <- as.numeric(quantile(y_s, q_s))
u_a <- as.numeric(quantile(y_a, q_a))
ex_s <- y_s[y_s > u_s]
ex_a <- y_a[y_a > u_a]

u_vec    <- c(u_s, u_a)
ymax_vec <- c(max(ex_s), max(ex_a))
y_list   <- list(ex_s, ex_a)

## ---------------------------
## 4) Densities & numerics
## ---------------------------
logsumexp <- function(v) { m <- max(v); m + log(sum(exp(v - m))) }

dhalfcauchy <- function(x, scale = 1, log = FALSE) {
  dens <- (2/(pi*scale)) * 1/(1 + (x/scale)^2)
  if (log) return(log(dens)); dens
}

# Truncated normal N(mean, sd^2) on (a,b)
dtnorm <- function(x, mean=0, sd=1, a=-Inf, b=Inf, log=FALSE) {
  Z <- pnorm((b-mean)/sd) - pnorm((a-mean)/sd)
  dens <- dnorm((x-mean)/sd)/sd / Z
  if (log) return(log(dens)); dens
}

# Finite-endpoint GP log-pdf (Weibull domain) for exceedance vector y
# σ = (u - y*) ξ, valid for ξ<0 and y ≤ y*
gp_logpdf_vec <- function(y, u, xi, ystar) {
  if (ystar <= u) return(-Inf)
  if (any(y > ystar)) return(-Inf)
  sigma <- (u - ystar) * xi
  if (sigma <= 0) return(-Inf)
  z <- 1 + (y - u)/(u - ystar)  # 1 - (u - y)/(y* - u) > 0 ⇒ y < y*
  if (any(z <= 0)) return(-Inf)
  n <- length(y)
  - n*log(sigma) - (1/xi + 1) * sum(log(z))
}

grid_quantile <- function(x, pdf, p) {
  dx  <- c(0, diff(x))
  cdf <- cumsum(pmax(pdf,0)*dx); cdf <- cdf / max(cdf)
  as.numeric(approx(cdf, x, xout = p, ties = "ordered")$y)
}

## ---------------------------
## 5) Grids (y*, ξ, hyperparameters)
## ---------------------------
make_ygrid <- function(ymax, u, mu0, sd0, n=Ny_per_group) {
  lo <- max(ymax + 1e-6, u + 1e-6)
  hi <- max(lo + 1e-3, mu0 + 4*sd0) + 0.3
  seq(lo, hi, length.out = n)
}

ygrid_list <- list(
  make_ygrid(ymax_vec[1], u_s, mu_y0, sd_y0, Ny_per_group),
  make_ygrid(ymax_vec[2], u_a, mu_y0, sd_y0, Ny_per_group)
)
dy1 <- diff(ygrid_list[[1]])[1]
dy2 <- diff(ygrid_list[[2]])[1]

xi_grid <- seq(-0.95, -0.02, length.out = Nxi)
dxi     <- diff(xi_grid)[1]

mu_y_grid   <- seq(mu_y_min,  mu_y_max,  length.out = N_mu_y)
tau_y_grid  <- exp(seq(log(tau_y_min), log(tau_y_max), length.out = N_tau_y))
mu_xi_grid  <- seq(mu_xi_min, mu_xi_max, length.out = N_mu_xi)
tau_xi_grid <- exp(seq(log(tau_xi_min), log(tau_xi_max), length.out = N_tau_xi))

dmu_y   <- diff(mu_y_grid)[1]
dtau_y  <- diff(tau_y_grid)[1]
dmu_xi  <- diff(mu_xi_grid)[1]
dtau_xi <- diff(tau_xi_grid)[1]

My <- N_mu_y * N_tau_y
Mx <- N_mu_xi * N_tau_xi

## ---------------------------
## 6) Likelihood tables A_j(y*, ξ)
## ---------------------------
build_A <- function(y_ex, u, ygrid, xi_grid) {
  Ny <- length(ygrid); Nxi <- length(xi_grid)
  M <- matrix(0.0, nrow = Ny, ncol = Nxi)
  for (k in 1:Ny) {
    ystar_k <- ygrid[k]
    logs <- vapply(xi_grid, function(xi) gp_logpdf_vec(y_ex, u, xi, ystar_k), numeric(1))
    M[k, ] <- ifelse(is.finite(logs), exp(logs), 0.0)
  }
  M
}
A1 <- build_A(ex_s, u_s, ygrid_list[[1]], xi_grid)
A2 <- build_A(ex_a, u_a, ygrid_list[[2]], xi_grid)

## ---------------------------
## 7) Basis matrices for priors (with Riemann weights)
## ---------------------------
build_Wy <- function(ygrid, mu_grid, tau_grid, dyi) {
  Ny <- length(ygrid); My <- length(mu_grid)*length(tau_grid)
  Wy <- matrix(0.0, nrow = Ny, ncol = My)
  col <- 1L
  for (it in seq_along(tau_grid)) {
    for (im in seq_along(mu_grid)) {
      Wy[, col] <- dnorm(ygrid, mean = mu_grid[im], sd = tau_grid[it]) * dyi
      col <- col + 1L
    }
  }
  Wy
}
build_Wxi <- function(xi_grid, mu_grid, tau_grid, dxi) {
  Nxi <- length(xi_grid); Mx <- length(mu_grid)*length(tau_grid)
  Wxi <- matrix(0.0, nrow = Nxi, ncol = Mx)
  col <- 1L
  for (it in seq_along(tau_grid)) {
    for (im in seq_along(mu_grid)) {
      Wxi[, col] <- dtnorm(xi_grid, mean = mu_grid[im], sd = tau_grid[it], a = -1, b = 0) * dxi
      col <- col + 1L
    }
  }
  Wxi
}

W_y1 <- build_Wy(ygrid_list[[1]], mu_y_grid, tau_y_grid, dy1)
W_y2 <- build_Wy(ygrid_list[[2]], mu_y_grid, tau_y_grid, dy2)
W_xi <- build_Wxi(xi_grid, mu_xi_grid, tau_xi_grid, dxi)

## (Density versions without Δ)
dens_y1 <- W_y1 / dy1
dens_y2 <- W_y2 / dy2
dens_x  <- W_xi / dxi

## ---------------------------
## 8) Group integrals on hyper-grids: M_j = t(W_yj) %*% A_j %*% W_xi
## ---------------------------
M1 <- t(W_y1) %*% A1 %*% W_xi
M2 <- t(W_y2) %*% A2 %*% W_xi
M1[M1 < eps] <- eps
M2[M2 < eps] <- eps

## ---------------------------
## 9) Hyperpriors (with Riemann weights included)
## ---------------------------
prior_y_vec <- as.vector(outer(
  dnorm(mu_y_grid, mean = mu_y0, sd = sd_y0) * dmu_y,
  dhalfcauchy(tau_y_grid, scale = hc_scale_y) * dtau_y,
  "*"
))
prior_xi_vec <- as.vector(outer(
  dnorm(mu_xi_grid, mean = mu_xi0, sd = sd_xi0) * dmu_xi,
  dhalfcauchy(tau_xi_grid, scale = hc_scale_xi) * dtau_xi,
  "*"
))
prior_y_vec[prior_y_vec < eps]   <- eps
prior_xi_vec[prior_xi_vec < eps] <- eps

log_prior_y  <- log(prior_y_vec)
log_prior_xi <- log(prior_xi_vec)

## ---------------------------
## 10) Hyperposterior surface: Post ∝ prior_y · prior_xi · Π_j M_j
## ---------------------------
logPost <- matrix(log_prior_y,  nrow = My, ncol = Mx, byrow = FALSE) +
  matrix(log_prior_xi, nrow = My, ncol = Mx, byrow = TRUE)  +
  log(M1) + log(M2)

norm_const <- logsumexp(as.vector(logPost))
Post <- exp(logPost - norm_const)  # My x Mx (rectangle-rule normalized)

## ---------------------------
## 11) Population displays
##   (a) Posterior for μ_y (shown on TL scale via exp)
##   (b) TRUE population predictive p_pop(y*) = ∫ N(y*; μ_y, τ_y^2) p(η|data) dη
## ---------------------------
# (a) p(μ_y | data) on TL scale via change-of-variables t=exp(μ_y)
Post_array        <- array(Post, dim = c(N_mu_y, N_tau_y, Mx))
post_mu_y_mass    <- apply(Post_array, 1, sum)
post_mu_y_mass    <- post_mu_y_mass / sum(post_mu_y_mass)

tl_grid_pop_mu    <- exp(mu_y_grid)
pdf_tl_pop_mu     <- (post_mu_y_mass / tl_grid_pop_mu)
pdf_tl_pop_mu     <- pdf_tl_pop_mu / trapz(tl_grid_pop_mu, pdf_tl_pop_mu)
pop_mu_map        <- tl_grid_pop_mu[ which.max(pdf_tl_pop_mu) ]
pop_mu_up         <- grid_quantile(tl_grid_pop_mu, pdf_tl_pop_mu, ci_level)

# (b) TRUE p_pop(y*) by mixing N(y*; μ_y, τ_y^2) over Post
make_pop_ygrid <- function(ymax_pool, u_pop, mu0=mu_y0, sd0=sd_y0, Ny=320) {
  lo <- max(ymax_pool + 1e-6, u_pop + 1e-6); hi <- max(lo + 1e-3, mu0 + 4*sd0) + 0.3
  seq(lo, hi, length.out = Ny)
}
y_pool   <- c(y_s, y_a)
u_pop    <- as.numeric(quantile(y_pool, max(q_s, q_a)))
ymax_pool<- max(c(ex_s, ex_a))
ygrid_pop<- make_pop_ygrid(ymax_pool, u_pop, Ny = 320)
dy_pop   <- diff(ygrid_pop)[1]

# Ny_pop x My density basis for y* given (μ_y, τ_y)
dens_y_pop <- build_Wy(ygrid_pop, mu_y_grid, tau_y_grid, dyi = dy_pop) / dy_pop  # Ny_pop x My
w_My       <- rowSums(Post)  # marginal over (μ_ξ, τ_ξ)
pdf_y_pop  <- as.numeric(dens_y_pop %*% w_My)
pdf_y_pop  <- pdf_y_pop / trapz(ygrid_pop, pdf_y_pop)

# Transform to TL scale
tl_grid_pop_true <- exp(ygrid_pop)
pdf_tl_pop_true  <- (pdf_y_pop / tl_grid_pop_true)
pdf_tl_pop_true  <- pdf_tl_pop_true / trapz(tl_grid_pop_true, pdf_tl_pop_true)
pop_true_map     <- tl_grid_pop_true[ which.max(pdf_tl_pop_true) ]
pop_true_up      <- grid_quantile(tl_grid_pop_true, pdf_tl_pop_true, ci_level)

## ---------------------------
## 12) Dataset-specific posteriors p(y*_j | all data)
##     Vectorized: dens[k] ∝ wy_row(k) • (Rj %*% Hj_row(k))
## ---------------------------
H1 <- A1 %*% W_xi              # Ny1 x Mx
H2 <- A2 %*% W_xi              # Ny2 x Mx

R1 <- (matrix(prior_y_vec,  nrow = My, ncol = Mx, byrow = FALSE)) *
  (matrix(prior_xi_vec, nrow = My, ncol = Mx, byrow = TRUE))  * M2
R2 <- (matrix(prior_y_vec,  nrow = My, ncol = Mx, byrow = FALSE)) *
  (matrix(prior_xi_vec, nrow = My, ncol = Mx, byrow = TRUE))  * M1

# Stabilize (scale cancels in normalization)
R1 <- R1 / max(R1)
R2 <- R2 / max(R2)

# Vectorized evaluation over all k at once:
post_y_density_vec <- function(dens_y_j, Hj, Rjs, ygrid) {
  # V = Rjs %*% t(Hj): My x Ny
  V <- Rjs %*% t(Hj)
  Ny <- length(ygrid)
  # dens[k] = sum_m dens_y_j[k,m] * V[m,k]
  dens <- rowSums(dens_y_j * t(V))
  dens / trapz(ygrid, dens)
}

pdf_y1 <- post_y_density_vec(dens_y1, H1, R1, ygrid_list[[1]])
pdf_y2 <- post_y_density_vec(dens_y2, H2, R2, ygrid_list[[2]])

to_tl_density <- function(ygrid, pdf_y) {
  tgrid <- exp(ygrid)
  pdf_t <- pdf_y / tgrid
  pdf_t <- pdf_t / trapz(tgrid, pdf_t)
  list(tgrid = tgrid, pdf_t = pdf_t)
}
dens_s <- to_tl_density(ygrid_list[[1]], pdf_y1)
dens_a <- to_tl_density(ygrid_list[[2]], pdf_y2)

sergio_map <- dens_s$tgrid[ which.max(dens_s$pdf_t) ]
sergio_up  <- grid_quantile(dens_s$tgrid, dens_s$pdf_t, ci_level)
allan_map  <- dens_a$tgrid[ which.max(dens_a$pdf_t) ]
allan_up   <- grid_quantile(dens_a$tgrid, dens_a$pdf_t, ci_level)

## ---------------------------
## 13) Prior overlay helpers
## ---------------------------
t_prior <- seq(300, 800, length.out = 4000)
prior_df <- data.frame(
  t = t_prior,
  d = dlnorm(t_prior, meanlog = mu_y0, sdlog = sd_y0)
)

make_single_plot <- function(tgrid, post, prior_df, title_lab, map_val, up_val, xlim_pair=c(300,700)) {
  ggplot() +
    geom_area(data = prior_df, aes(t, d), fill = "grey70", alpha = 0.18) +
    geom_line(data = prior_df, aes(t, d), color = "grey50", linewidth = 0.8, alpha = 0.9) +
    geom_line(data = data.frame(t=tgrid, d=post), aes(t, d), color = "darkblue", linewidth = 1.2) +
    geom_vline(xintercept = map_val, color = "purple",  linetype = "dashed",  linewidth = 1.05) +
    geom_vline(xintercept = up_val,  color = "orange",  linetype = "dotdash", linewidth = 1.0) +
    geom_vline(xintercept = stokes_tl_cm, color = "firebrick", linetype = "dashed", linewidth = 1.0) +
    annotate("text", x = map_val, y = max(post, na.rm=TRUE)*0.08,
             label = sprintf("%.0f cm", map_val), angle = 90, vjust = -0.4,
             size = 4.2, fontface = "bold", color = "purple") +
    labs(x = "TL* (cm)", y = "Density", title = title_lab,
         caption = "Grey: prior on TL* (log-normal). Blue: posterior (quadrature).") +
    theme_science_polished + xlim(xlim_pair[1], xlim_pair[2])
}

## ---------------------------
## 14) Plots
## ---------------------------
p_sergio <- make_single_plot(
  tgrid = dens_s$tgrid, post = dens_s$pdf_t, prior_df = prior_df,
  title_lab = "TL* — Sergio dataset",
  map_val = sergio_map,
  up_val  = sergio_up
)
ggsave(file.path(FIG_DIR, "hier_Sergio_TLstar.png"),
       p_sergio, dpi = 600, width = 7, height = 5, units = "in")

p_allan <- make_single_plot(
  tgrid = dens_a$tgrid, post = dens_a$pdf_t, prior_df = prior_df,
  title_lab = "TL* — Allan dataset",
  map_val = allan_map,
  up_val  = allan_up
)
ggsave(file.path(FIG_DIR, "hier_Allan_TLstar.png"),
       p_allan, dpi = 600, width = 7, height = 5, units = "in")

# Overlay
p_both <- ggplot() +
  geom_area(data = prior_df, aes(t, d), fill = "grey70", alpha = 0.18) +
  geom_line(data = prior_df, aes(t, d), color = "grey50", linewidth = 0.8, alpha = 0.9) +
  geom_line(data = data.frame(t=dens_s$tgrid, d=dens_s$pdf_t),
            aes(t, d, color = "Sergio"), linewidth = 1.25) +
  geom_line(data = data.frame(t=dens_a$tgrid, d=dens_a$pdf_t),
            aes(t, d, color = "Allan"),  linewidth = 1.25) +
  scale_color_manual(values = c("Sergio"="darkblue","Allan"="#2a9df4")) +
  geom_vline(xintercept = stokes_tl_cm, color = "firebrick", linetype = "dashed") +
  geom_vline(xintercept = sergio_map,   color = "purple",   linetype = "dashed") +
  geom_vline(xintercept = allan_map,    color = "purple",   linetype = "dashed") +
  geom_vline(xintercept = sergio_up,    color = "orange",   linetype = "dotdash") +
  geom_vline(xintercept = allan_up,     color = "orange",   linetype = "dotdash") +
  labs(x = "TL* (cm)", y = "Density", color = NULL,
       caption = "Grey: prior (wide grid); colored: dataset posteriors.") +
  theme_science_polished + theme(legend.position = "top") + xlim(300,700)
ggsave(file.path(FIG_DIR, "hier_Overlay_TLstar.png"),
       p_both, dpi = 600, width = 7.5, height = 5.2, units = "in")

# Population displays: (i) μ_y and (ii) TRUE p_pop(y*)
p_pop_mu <- make_single_plot(
  tgrid = tl_grid_pop_mu, post = pdf_tl_pop_mu, prior_df = prior_df,
  title_lab = "Population TL* — via exp(μ_y)",
  map_val = pop_mu_map, up_val = pop_mu_up
)
ggsave(file.path(FIG_DIR, "hier_Pop_TLstar_mu.png"),
       p_pop_mu, dpi = 600, width = 7, height = 5, units = "in")

p_pop_true <- make_single_plot(
  tgrid = tl_grid_pop_true, post = pdf_tl_pop_true, prior_df = prior_df,
  title_lab = "Population TL* — predictive p_pop(y*)",
  map_val = pop_true_map, up_val = pop_true_up
)
ggsave(file.path(FIG_DIR, "hier_Pop_TLstar_true.png"),
       p_pop_true, dpi = 600, width = 7, height = 5, units = "in")

## ---------------------------
## 15) Console summary
## ---------------------------
cat("\n--- Hierarchical TL* (quadrature) summaries ---\n")
cat(sprintf("Sergio:    MAP = %.1f cm | %d%% upper = %.1f cm\n",
            sergio_map, round(100*ci_level), sergio_up))
cat(sprintf("Allan:     MAP = %.1f cm | %d%% upper = %.1f cm\n",
            allan_map,  round(100*ci_level), allan_up))
cat(sprintf("Pop (exp μ_y): MAP = %.1f cm | %d%% upper = %.1f cm\n",
            pop_mu_map, round(100*ci_level), pop_mu_up))
cat(sprintf("Pop (true y*): MAP = %.1f cm | %d%% upper = %.1f cm\n\n",
            pop_true_map, round(100*ci_level), pop_true_up))

## =========================================================
## 16) Deterministic tail probabilities (datasets + population)
## =========================================================
S_weibull_mat <- function(y_log, u_log, ystar_grid, xi_grid) {
  Ny <- length(ystar_grid); Nxi <- length(xi_grid)
  YS <- matrix(ystar_grid, nrow = Ny, ncol = Nxi, byrow = FALSE)
  XI <- matrix(xi_grid,    nrow = Ny, ncol = Nxi, byrow = TRUE)
  ratio <- (YS - y_log) / (YS - u_log) # valid for y < y*
  S <- matrix(0.0, nrow = Ny, ncol = Nxi)
  ok <- (YS > y_log) & is.finite(ratio) & (ratio > 0)
  S[ok] <- ratio[ok]^(-1 / XI[ok])
  S[!is.finite(S)] <- 0.0
  S
}

weighted_quantile <- function(values, weights, probs = c(0.05, 0.5, 0.95)) {
  o <- order(values); v <- values[o]; w <- pmax(weights[o], 0)
  W <- sum(w); if (!is.finite(W) || W <= 0) return(rep(NA_real_, length(probs)))
  cw <- cumsum(w) / W
  as.numeric(approx(x = cw, y = v, xout = probs, ties = "ordered", rule = 2)$y)
}

tail_eval_dataset_quad <- function(target_cm,
                                   A_j, dens_y_j, Rjs, ygrid, xi_grid, u_log,
                                   dy, dxi, Nu, N,
                                   qlo = 0.05, qhi = 0.95) {
  K_mid  <- dens_y_j %*% Rjs          # Ny x Mx
  K      <- K_mid %*% t(dens_x)       # Ny x Nxi
  Joint  <- pmax(K * A_j, 0) * dy * dxi
  Z      <- sum(Joint)
  if (!is.finite(Z) || Z <= 0) stop("Joint normalization failed (dataset case).")
  W_joint <- Joint / Z
  
  eval_one <- function(t_cm) {
    ylog <- log(t_cm)
    S    <- S_weibull_mat(ylog, u_log, ygrid, xi_grid)
    p_c  <- sum(W_joint * S)
    qs   <- weighted_quantile(as.vector(S), as.vector(W_joint), c(qlo, qhi))
    data.frame(
      target_cm = t_cm,
      p_cond    = pmin(pmax(p_c, 0), 1),
      p_cond_lo = pmin(pmax(qs[1], 0), 1),
      p_cond_hi = pmin(pmax(qs[2], 0), 1),
      p_full    = (Nu/N) * p_c,
      p_full_lo = (Nu/N) * qs[1],
      p_full_hi = (Nu/N) * qs[2]
    )
  }
  do.call(rbind, lapply(as.numeric(target_cm), eval_one))
}

tail_eval_sergio_quad <- function(target_cm) {
  Nu <- length(ex_s); N <- length(y_s)
  tail_eval_dataset_quad(target_cm, A1, dens_y1, R1, ygrid_list[[1]], xi_grid, u_s,
                         dy1, dxi, Nu, N)
}
tail_eval_allan_quad <- function(target_cm) {
  Nu <- length(ex_a); N <- length(y_a)
  tail_eval_dataset_quad(target_cm, A2, dens_y2, R2, ygrid_list[[2]], xi_grid, u_a,
                         dy2, dxi, Nu, N)
}

# Population predictive: (y*, ξ) ~ dens_y_pop %*% Post %*% t(dens_x)
tail_eval_population_quad <- function(target_cm, Ny_pop = 320, qlo = 0.05, qhi = 0.95) {
  y_pool <- c(y_s, y_a); N <- length(y_pool)
  u_pop  <- as.numeric(quantile(y_pool, max(q_s, q_a)))
  Nu     <- sum(y_pool > u_pop)
  
  ygrid_pop <- make_pop_ygrid(ymax_pool, u_pop, Ny = Ny_pop)
  dy_pop    <- diff(ygrid_pop)[1]
  dens_y_pop <- build_Wy(ygrid_pop, mu_y_grid, tau_y_grid, dyi = dy_pop) / dy_pop
  
  K_mid  <- dens_y_pop %*% Post       # Ny_pop x Mx
  K      <- K_mid %*% t(dens_x)       # Ny_pop x Nxi
  Joint  <- pmax(K, 0) * dy_pop * dxi
  Z      <- sum(Joint)
  if (!is.finite(Z) || Z <= 0) stop("Joint normalization failed (population case).")
  W_joint <- Joint / Z
  
  eval_one <- function(t_cm) {
    ylog <- log(t_cm)
    S    <- S_weibull_mat(ylog, u_pop, ygrid_pop, xi_grid)
    p_c  <- sum(W_joint * S)
    qs   <- weighted_quantile(as.vector(S), as.vector(W_joint), c(qlo, qhi))
    data.frame(
      target_cm = t_cm,
      p_cond    = pmin(pmax(p_c, 0), 1),
      p_cond_lo = pmin(pmax(qs[1], 0), 1),
      p_cond_hi = pmin(pmax(qs[2], 0), 1),
      p_full    = (Nu/N) * p_c,
      p_full_lo = (Nu/N) * qs[1],
      p_full_hi = (Nu/N) * qs[2]
    )
  }
  do.call(rbind, lapply(as.numeric(target_cm), eval_one))
}

## ---------------------------
## 17) Example tail calls
## ---------------------------
targets <- c(400, 425, 450, 475)

res_sergio_q <- tail_eval_sergio_quad(targets)
res_allan_q  <- tail_eval_allan_quad(targets)
res_pop_q    <- tail_eval_population_quad(targets)

print(res_sergio_q)
print(res_allan_q)
print(res_pop_q)

# Stokes line (450 cm)
stokes <- 450
cat(sprintf("\nSergio (quad):  p_cond=%.3e [%.3e, %.3e] ; p_full=%.3e\n",
            subset(res_sergio_q, target_cm==stokes)$p_cond,
            subset(res_sergio_q, target_cm==stokes)$p_cond_lo,
            subset(res_sergio_q, target_cm==stokes)$p_cond_hi,
            subset(res_sergio_q, target_cm==stokes)$p_full))
cat(sprintf("Allan  (quad):  p_cond=%.3e [%.3e, %.3e] ; p_full=%.3e\n",
            subset(res_allan_q, target_cm==stokes)$p_cond,
            subset(res_allan_q, target_cm==stokes)$p_cond_lo,
            subset(res_allan_q, target_cm==stokes)$p_cond_hi,
            subset(res_allan_q, target_cm==stokes)$p_full))
cat(sprintf("Popul. (quad):  p_cond=%.3e [%.3e, %.3e] ; p_full=%.3e\n\n",
            subset(res_pop_q, target_cm==stokes)$p_cond,
            subset(res_pop_q, target_cm==stokes)$p_cond_lo,
            subset(res_pop_q, target_cm==stokes)$p_cond_hi,
            subset(res_pop_q, target_cm==stokes)$p_full))
