...
E1 <- O1 & (x1 > u1)  # CF exceed
E2 <- O2 & (x2 > u2)  # CH exceed
B1 <- O1 & !E1        # CF observed but below u1 ⇒ censored at u1
B2 <- O2 & !E2        # CH observed but below u2 ⇒ censored at u2

# Full pattern per specimen (for debugging / tables if needed)
pattern_df <- tibble(
...
make_pair_plot <- function(t1 = "CF", t2 = "CH") {
  ...
  # points where both traits observed
  dd_dot <- tibble(
  ...
  # vertical segments: CF exceed, CH missing
  dd_v <- tibble(
  ...
  # horizontal segments: CH exceed, CF missing
  dd_h <- tibble(
