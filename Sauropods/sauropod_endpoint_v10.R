...
# Global settings
# ---------------------------
ci_level      <- 0.9          # keep 90% CIs for sauropods
u_scan_lo_q   <- 0.20
u_scan_hi_q   <- 0.99
u_scan_n      <- 50
min_ex_mrl    <- 5
...
B_boot <- 10000L   # parametric bivariate bootstrap replicates
...
cf_samp_df <- cf_samp_df |> dplyr::filter(CF < 1e4)
ch_samp_df <- ch_samp_df |> dplyr::filter(CH < 1e4)
...
# ------------------------------------------------------------
# CF* bootstrap distribution (histogram only)
# ------------------------------------------------------------
p_cf <- ggplot(cf_samp_df, aes(x = CF)) +
  geom_histogram(
    bins  = 40,
    fill  = "lightblue",
    color = "black",
  ...
  labs(x = "CF endpoint (mm)", y = "Count") +
  coord_cartesian(xlim = cf_xlim) +
  theme_science_polished

# ------------------------------------------------------------
# CH* bootstrap distribution (histogram only)
# ------------------------------------------------------------
p_ch <- ggplot(ch_samp_df, aes(x = CH)) +
  geom_histogram(
    bins  = 40,
    fill  = "lightgreen",
    color = "black",
