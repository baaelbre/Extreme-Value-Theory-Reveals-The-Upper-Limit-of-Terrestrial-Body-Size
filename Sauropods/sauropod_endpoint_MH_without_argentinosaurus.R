############################################################
#  Title: POT analysis on log CFH (without Argentinosaurus)  
...
load_cfh_data <- function(path) {
  read_excel(path, .name_repair = "minimal") %>%           # keep raw names
...
df_cfh_full <- load_cfh_data(
  "Data/DEmic23_updated_Supplemental_Data_withPubYear.xlsx"
)

## --- 2.1  REMOVE LARGEST SPECIMEN (Argentinosaurus) -------------
largest_row <- which.max(df_cfh_full$sum_circ_mm)

df_cfh      <- df_cfh_full[-largest_row, ]  # training set
arg_row     <- df_cfh_full[ largest_row, ]  # keep for validation if needed

## --- 2.2  VECTORS FOR ANALYSIS ----------------------------------
log_circ <- df_cfh$log_circ            # training vector (largest removed)
x_max    <- max(log_circ)              # now excludes Argentinosaurus
...
threshold_opt <- th_tbl$threshold[21]  
