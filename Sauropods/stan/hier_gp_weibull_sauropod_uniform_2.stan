...
data {
  ...
  // ---- Prior support for endpoint (dataset-level) ----
  real y_min;                // user lower cap for y*
  real y_max;                // legacy/user cap (kept for safety)
  real y_cap;                // hard cap for y* prior (e.g., 8.62 ~ 1000 t)

  // ---- Hyperprior on mu_y: Normal(mu_y0, sd_mu_y) TRUNCATED to [mu_y_lo, mu_y_hi] ----
  real mu_y0;                // e.g., 8.15
  real<lower=0> sd_mu_y;     // e.g., 0.2–0.6
  real mu_y_lo;              // = 7.7
  real mu_y_hi;              // = 8.62
...
transformed data {
  ...
  if (!(mu_y_lo < mu_y_hi))
    reject("mu_y_lo must be < mu_y_hi.");
}

parameters {
  // ---- Hyperparameters ----
  // Truncated Normal hyperprior support by declaration bounds:
  real<lower=mu_y_lo, upper=mu_y_hi> mu_y;
  real<lower=0> tau_y;
...
model {
  // ---- Truncated-Normal hyperprior on mu_y in [mu_y_lo, mu_y_hi] ----
  target += normal_lpdf(mu_y | mu_y0, sd_mu_y)
            - log_diff_exp(normal_lcdf(mu_y_hi | mu_y0, sd_mu_y),
                           normal_lcdf(mu_y_lo | mu_y0, sd_mu_y));

  // SD prior for tau_y (menu)
  ...
  // Hyperprior on mu_xi and SD prior on tau_xi
  ...
  // ---- Dataset-level truncated priors ----
