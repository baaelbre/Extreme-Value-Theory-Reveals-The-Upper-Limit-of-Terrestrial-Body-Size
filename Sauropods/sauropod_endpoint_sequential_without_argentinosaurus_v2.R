...
#  Date  : 2025-09-09
#  Notes : Argentinosaurus removed from dataset & overlays
############################################################
...
## ---------------------------
## 2) Data ingest (exclude Argentinosaurus)
## ---------------------------
dat_path <- "Data/DEMic23_updated_Supplemental_Data_withPubYear.xlsx"

# Keep 'species' so we can filter; drop NAs; exclude Argentinosaurus
df <- read_excel(dat_path) |>
  dplyr::select(species = any_of(c("species","Species","taxon","Taxon")),
                `hum+fem circ (mm)`, `publication year`) |>
  dplyr::rename(sum_circ_mm = `hum+fem circ (mm)`,
                year        = `publication year`) |>
  tidyr::drop_na()

# In case the sheet lacks a species column, create a placeholder (won't filter anything)
if (!"species" %in% names(df)) df$species <- NA_character_

df <- df |>
  filter(is.na(species) | species != "Argentinosaurus")  # <<< CHANGED
...
## ---------------------------
logprior_xi <- function(xi) if (xi < 0) 0 else -Inf
...
## ---------------------------
## 6) Build overlays (drop Argentinosaurus row)
## ---------------------------
top10_data <- data.frame(
  species = c("Ruyangosaurus","Turiasaurus","Yunmenglong","Australotitan",
              ...
              "Brachiosaurus"),
  year = c(2017,2006,2013,2021,2017,2014,2016,2024,1903),
  sum_circ_mm = c(1639,1672,1687,1692,1694,1695,1704,1827,1870),
  mass_quad   = c(43.4,45.7,46.7,47.1,47.2,47.3,48.0,57.5,61.0),
  pi_lower_quad = c(19.8,20.8,21.2,21.4,21.4,21.5,21.7,25.8,27.3),
  pi_upper_quad = c(95.0,100.0,103.0,104.0,104.0,104.0,106.0,128.0,137.0)
)
top10_data$type <- "Specimen"

# Mass estimates for ALL exceedances (y > u0) using quadratic mapping (no PI bars; points only)
df$mass_quad_all <- exp(map_logM_cq(df$log_circ)) / 1e6
exceed_pts <- df |> dplyr::filter(log_circ > u0) |> dplyr::select(year, mass_quad_all)

# palette for named specimens (no Argentinosaurus)
species_colors <- c(
...
## ---------------------------
## 7) Plot — Top-9 (excl. Argentinosaurus) + all exceedances + Extremosaurus
## ---------------------------

# Extremosaurus MAP line
ext_map <- results |>
  dplyr::transmute(year, mass = map_mass_tons, species = "Extremosaurus")

# Legend order without Argentinosaurus
legend_order <- c(
  "Ruyangosaurus","Turiasaurus","Yunmenglong","Australotitan",
  "Patagotitan","Dreadnoughtus","Notocolossus","Chucarosaurus",
  "Brachiosaurus","Extremosaurus"
)
...
p_mass <- ggplot() +
  ...
  scale_color_manual(values = species_colors, limits = legend_order, breaks = legend_order) +
...
p_mass
ggsave(file.path(FIG_DIR, "mass_sequential_map_no_argentinosaurus.png"),
...
p_mass_supplement
ggsave(file.path(FIG_DIR, "mass_sequential_supplement_no_argentinosaurus.png"),
       p_mass_supplement, dpi = 600, w = 7, h = 5, units = "in")

## ---------------------------
## 7b) Log-scale version
## ---------------------------
p_mass_log <- ggplot() +
  geom_point(
    data = exceed_pts,
...
p_mass_log
ggsave(file.path(FIG_DIR, "mass_sequential_uniform_improper_one_sided_log_no_argentinosaurus.png"),
       p_mass_log, dpi = 600, w = 7, h = 5, units = "in")
ggsave(file.path(FIG_DIR, "mass_sequential_uniform_improper_one_sided_log_no_argentinosaurus.pdf"),
